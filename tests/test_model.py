...
class TestTransformerModel:
    ...
    def test_rotary_positional_embeddings(self):
        ...
        assert torch.allclose(cos_sin_sum, torch.ones_like(cos_sin_sum), atol=1e-6)
    ...
    def test_model_parameters(self):
        ...
        expected_min = (
            ...
            config["num_layers"] * config["hidden_size"] * 4 +  # transformer blocks
...
class TestSoftmaxVariants:
    ...
    def test_sparsemax(self):
        ...
        for logits in test_cases:
            ...
            # Check that probabilities sum to 1
            prob_sums = probs.sum(dim=-1)
            assert torch.allclose(prob_sums, torch.ones_like(prob_sums), atol=1e-3)
