...
from tests.conftest import set_seed, TestConfig
...
class TestModularArithmeticDataset:
    ...
    def test_modular_arithmetic_correctness(self):
        ...
        for task_type, expected_func in test_cases:
            ...
            for i, (a, b, result) in enumerate(dataset.data[:100]):
                ...
                assert result == expected, f"Task {task_type}: {a} op {b} = {result}, expected {expected}"
    ...
    def test_parity_task(self):
        ...
        for i, (binary_str, _, parity) in enumerate(dataset.data[:100]):
            ...
            assert parity == expected_parity, f"Parity mismatch for {binary_str}"
    ...
    def test_gcd_task(self):
        ...
        for i, (a, b, result) in enumerate(dataset.data[:100]):
            ...
            assert result == expected_gcd, f"GCD mismatch: gcd({a}, {b}) = {result}, expected {expected_gcd}"
    ...
    def test_division_task(self):
        ...
        for i, (a, b, result) in enumerate(dataset.data[:100]):
            if b != 0:
                ...
                assert check_result == a, f"Division check failed: {result} * {b} ≡ {check_result} (mod {dataset.modulus}), expected {a}"
    ...
    def test_edge_cases(self):
        ...
        with pytest.raises(ValueError, match="Train split must be between 0 and 1"):
    ...
    def test_memory_efficiency(self):
        ...
        estimated_size = len(dataset.data) * 3 * 8  # 3 ints per sample, 8 bytes per int
